            ...
            <div class="card mb-4">
                <div class="card-header pb-0">
                    <div class="d-flex bd-highlight mb-3">
                        <h6 class="me-auto p-2 bd-highlight">Quản lý tài chính học sinh:
                                        ...
                                        @foreach ($dates as $date)
                                            ...
                                            @foreach ($lessons as $lesson)
                                                @if (\Carbon\Carbon::parse($lesson->created_at)->month == explode('/', $date)[0])
                                                    <tr>
                                                        ...
                                                        <td class="price-{{ $date }}">
                                                            @if (\App\Models\Attendance::where('lesson_id', $lesson->lesson_id)->where('student_id', $user->id)->where('status', 0)->exists())
                                                                {{ number_format($lesson->price_per_student) }}
                                                            @else
                                                                @php
                                                                    $lesson->price_per_student = 0;
                                                                @endphp
                                                                0
                                                            @endif
                                                        </td>
                                                        @php
                                                            $total_price += $lesson->price_per_student;
                                                        @endphp
                                                        <td>
                                                        </td>
                                                    </tr>
                                            ...
                                            <tr>
                                                ...
                                                <td style="border-width: 1px">
                                                    ...
                                                    @endphp
                                                    @if (\App\Models\StudentPayment::where('status', 0)->where('student_id', $user->id)->where('date', $date)->exists())
                                                                ...
                                                                <div class="modal-dialog">
                                                                    <div class="modal-content" style="width:150%">
                                                                        ...
                                                                        <div class="modal-body">
                                                                            <p>Bạn có sửa trạng thái thành chưa đóng tiền
                                                                                không
                                                                                ?</p>
                                                                        </div>
                                                                        <div class="modal-footer">
                                                                            ...
                                                                            <form
                                                                                action="{{ route('payment.student-update-status', [$user->id, $dateMonth, $dateYear]) }}"
                                                                                method="POST">
                                                                                @csrf
                                                                                @method('PUT')
                                                                                <button type="submit"
                                                        ...
                                                        @endif
                                                    @elseif (\App\Models\StudentPayment::where('status', 1)->where('student_id', $user->id)->where('date', $date)->exists())
                                                        <a style="cursor: pointer"
                                                            class="btn btn-secondary not-approved badge badge-sm bg-gradient-secondary"
                                                            data-bs-toggle="modal"
                                                            data-bs-target="#updateStatusModal-{{ $dateMonth . $dateYear }}"
                                                            class="tt-icon-btn">Chưa đóng</a>
                                                        @if (Auth::user()->role_id == 1)
                                                            <div class="modal fade"
                                                                id="updateStatusModal-{{ $dateMonth . $dateYear }}"
                                                                tabindex="-1" aria-labelledby="exampleModalLabel"
                                                                ...
                                                                <div class="modal-dialog">
                                                                    <div class="modal-content" style="width:150%">
                                                                        ...
                                                                        <div class="modal-body">
                                                                            <p>Bạn có sửa trạng thái thành đã đóng tiền
                                                                                không
                                                                                ?</p>
                                                                        </div>
                                                                        <div class="modal-footer">
