...
@section('content')
    @include('layouts.navbars.auth.topnav', ['title' => 'Danh sách tin tức bị ẩn'])
    <style>
        .overflow {
            display: -webkit-box;
            max-height: 3.2rem;
        ...
        <div class="col-12">
            <div class="card mb-4">
                <div class="card-header pb-0">
                    <div>
                        <a href="{{ URL::signedRoute('news.management') }}" style="color: blue; font-size: 15px"><i
                                class="fa-solid fa-arrow-left-long"></i> Trang trước</a>
                    </div>
                    <div class="d-flex bd-highlight mb-3">
                        <h6 class="me-auto p-2 bd-highlight">Danh sách tin tức bị ẩn</h6>
                        @if (count($news) > 0)
                            <form action="{{ route('news.restore-all') }}" method="POST">
                                @csrf
                                <button class="btn btn-primary btn-sm ms-auto button-float" style="margin: 0 20px;"
                                    class="p-2 bd-highlight">Khôi phục toàn bộ tin tức</button>
                            </form>
                        @endif
                    </div>
                    <form method="GET">
                        ...
                        <div class="input-group search" style="margin: 10px 0 20px 0;">
                            ...
                            <input type="text" name="q" class="form-control" value="{{ request()->get('q') }}"
                                placeholder="Tìm kiếm tin tức bị ẩn...">
                        </div>
                    </form>
                </div>
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="align-middle">
                                                        <a style="cursor: pointer" data-bs-toggle="modal"
                                                            data-bs-target="#restoreUserModal-{{ $new->id }}"
                                                            class="tt-icon-btn"><i
                                                                ...
                                                                <div class="modal-content">
                                                                    <div class="modal-header">
                                                                        <h1 class="modal-title fs-5 d-flex p-2"
                                                                            ...
                                                                            Xóa hoàn toàn tin tức</h1>
                                                                        <button type="button" class="btn-close"
                                                                            data-bs-dismiss="modal"
                                                                            aria-label="Close"></button>
                                                                    </div>
                                                                    <div class="modal-body">
                                                                        Bạn có chắc muốn xóa hoàn toàn tin tức này không
                                                                        ?
                                                                    </div>
                                                                    <div class="modal-footer">
                                                                        ...
                                                                        <form
                                                                            action="{{ route('news.force-delete', [$new->id]) }}"
                                                                            method="POST">
                                                                            @csrf
                                                                            @method('DELETE')
                                                                            <button type="submit"
                                                        ...
                                                        <div class="modal fade" id="restoreUserModal-{{ $new->id }}"
                                                            tabindex="-1" aria-labelledby="exampleModalLabel"
                                                            aria-hidden="true">
                                                            <div class="modal-dialog">
                                                                <div class="modal-content">
                                                                    <div class="modal-header">
                                                                        <h1 class="modal-title fs-5 d-flex p-2"
                                                                            id="exampleModalLabel">
                                                                            Khôi phục tin tức</h1>
                                                                        <button type="button" class="btn-close"
                                                                            data-bs-dismiss="modal"
                                                                            aria-label="Close"></button>
                                                                    </div>
                                                                    <div class="modal-body">
                                                                        Bạn có chắc muốn khôi phục tin tức này không ?
                                                                    </div>
                                                                    <div class="modal-footer">
                                                                        <button type="button" class="btn btn-secondary"
                                                                            data-bs-dismiss="modal">Đóng</button>
                                                                        <form
                                                                            action="{{ route('news.restore', [$new->id]) }}"
                                                                            method="POST">
                                                                            @csrf
                                                                            <button type="submit"
