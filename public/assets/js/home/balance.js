...
function doBalance() {
    ...
    try {
        ...
        if (e instanceof ParseError) { // Error message object with start and possibly end character indices
            msgElem.textContent = "Vui lòng nhập đúng phương trình cần cân bằng";
            const start = e.start;
...
function solve(matrix) {
    ...
    if (i == matrix.numRows - 1)
        throw new RangeError("Các nguyên tố có trong chất tham gia phải xuất hiện ở sản phẩm và ngược lại"); // Unique solution with all coefficients zero
...
function extractCoefficients(matrix) {
    ...
    if (cols - 1 > rows || matrix.get(cols - 2, cols - 2) == 0)
        throw new RangeError("Phương trình có vô số nghiệm");
    let lcm = 1;
    ...
    if (coefs.every(x => x == 0))
        throw new RangeError("Assertion error: Các nguyên tố có trong chất tham gia phải xuất hiện ở sản phẩm và ngược lại");
    return coefs;
}
...
function checkAnswer(eqn, coefs) {
    ...
    if (coefs.every(isZero))
        throw new Error("Assertion error: Các nguyên tố có trong chất tham gia phải xuất hiện ở sản phẩm và ngược lại");
...
function checkOverflow(x) {
    if (Math.abs(x) >= INT_MAX)
        throw new RangeError("Phương trình cẩn hệ số quá lớn để cân bằng");
    return x;
}
